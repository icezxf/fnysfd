此为终极版！搞定了音视频信息提取 豆瓣评分注入(面板带了豆瓣开关 去掉了海报墙预取那些不用的) 面板管理优化过了 
...
- 用户浏览海报墙时，拦截列表 API 响应，自动批量预取所有电影的 PlaybackInfo
- 进入详情页时直链已就绪，点击播放直接命中缓存
- 可配置并发数（默认 4）和单次预取上限（默认 50）

**全库扫描预取**

- 定时扫描整个媒体库，批量预取所有影片的 PlaybackInfo（支持电影 + 电视剧）
- 定时扫描时间可配置（如凌晨 3:00），或启动后立即扫描
- 电视剧自动查询季列表，每季预取前 N 集（可配置，默认 5 集）
- 面板支持手动触发扫描和查看扫描状态
...
| 保护措施 | 说明 |
|----------|------|
| 信号量并发控制 | 扫描并发默认 2，海报墙默认 4，可配置 |
| 每页间隔延迟 | 默认 500ms，可配置 0-60000ms |
| 总项目数上限 | 安全阀 10000 项，超过自动停止 |
| 内存检查 + GC | 每批处理后检查，超 400MB 自动 GC 暂停 |
| CAS 防并发 | atomic.Bool 防止多次扫描同时运行 |
| 优雅停止 | 随时可中断，等待在途任务完成 |
| 认证复用 | 从用户请求自动捕获认证头，供后台扫描使用 |
...
# Docker 构建
docker build -t fnysfd:3.4.0 .

# 或直接编译
go build -ldflags="-s -w -X main.version=3.4.0" -o fnysfd ./cmd
```
...
# ===== 媒体信息预取（v3.4.0 新增）=====
# 海报墙预取：浏览海报墙时自动批量预取电影 PlaybackInfo
enable_poster_prefetch: false       # 默认关闭
poster_prefetch_concurrency: 4      # 并发数（建议2-8）
poster_prefetch_max_items: 50       # 单次预取上限

# 全库扫描预取：定时扫描整个媒体库
enable_library_scan: false          # 默认关闭
library_scan_cron: "03:00"          # 定时扫描 HH:MM
library_scan_on_start: false        # 启动后立即扫描
library_scan_concurrency: 2         # 扫描并发（建议1-4）
library_scan_interval_ms: 500       # 每页间隔毫秒
...
│   │   ├── auth_store.go       # 认证信息缓存（供后台扫描复用）
│   │   ├── batch_prefetch.go   # 批量预取引擎（信号量并发控制）
│   │   ├── poster_prefetch.go  # 海报墙预取器（拦截列表响应）
│   │   └── library_scan.go     # 全库扫描器（定时/手动扫描）
│   ├── handler/
...
### Q: 起播报 3003 错误怎么办？
3003 通常是因为 CDN 未预热完成。v3.3.11 已修复电影跳过预热的问题，确保所有媒体类型都参与 CDN 预热。v3.4.0 新增海报墙预取和全库扫描预取，可提前批量预取 PlaybackInfo，进一步减少首次播放等待。如果仍有问题，尝试返回详情页重新进入。
...
- **海报墙预取**：用户浏览海报墙时自动拦截列表响应，批量预取所有电影的 PlaybackInfo
- **全库扫描预取**：定时或手动扫描整个媒体库，批量预取所有影片的 PlaybackInfo（电影 + 电视剧前 N 集）
- **认证信息缓存**：从用户请求自动捕获 Emby 认证头，供后台扫描任务复用
- **批量预取引擎**：信号量并发控制 + 去重 + 优雅停止，海报墙和全库扫描共用
- **多层性能保护**：并发限制、每页间隔、总项目数上限（10000）、内存检查 + GC（400MB 阈值）、CAS 防并发
- **面板集成**：新增预取配置区 + 手动扫描触发 + 扫描状态实时查看
- 修复批量预取未传递认证头的 bug
