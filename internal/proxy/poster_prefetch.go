...
func (p *PosterPrefetcher) HandleListResponse(resp *http.Response, body []byte, userID string) {
	...
	// 处理可能的 gzip 压缩（反向代理转发的请求可能包含 Accept-Encoding: gzip，
	// 导致飞牛返回 gzip 压缩的 JSON）
	data := body
	...
	// 3. 解析 JSON: {"Items": [{"Id":"xxx","Name":"xxx","Type":"Movie"}, ...]}
	...
	if len(items) == 0 {
		p.logger.Debug("🖼️ [海报墙预取] 无 Movie 类型项目 (列表共 %d 项)", len(listResp.Items))
		return
	}

	p.logger.Info("🖼️ [海报墙预取] 提取到 %d 部电影（列表共 %d 项），开始批量预取",
		len(items), len(listResp.Items))

	// 6. 异步执行批量预取（使用 authStore 中的认证头）
	// authStore 在第 1 步已从 resp.Request 捕获认证信息
...
// 当反向代理转发的请求包含 Accept-Encoding: gzip 时，飞牛返回 gzip 压缩的 JSON，
// 需要解压后才能解析。
