...
// Start 加载磁盘缓存 + 启动定期写盘
func (dp *DoubanProvider) Start() {
	if !config.Global.GetEnableDoubanRating() {
		dp.logger.Info("🎬 [豆瓣评分] 功能未开启")
		return
	}

	if err := dp.loadDB(); err != nil {
		dp.logger.Warn("🎬 [豆瓣评分] 加载缓存失败: %v", err)
	} else {
		dp.indexMu.RLock()
		n := len(dp.index)
		dp.indexMu.RUnlock()
		dp.logger.Info("🎬 [豆瓣评分] 缓存已加载: %d 条", n)
	}
	...
	dp.logger.Info("🎬 [豆瓣评分] 组件就绪（扫描由 LibraryScanner 驱动）")
}
...
func (dp *DoubanProvider) FetchMovieOrSeries(ctx context.Context, imdbID, name string, year int, itemType string) {
	...
	if imdbID != "" {
		...
		if err == nil && rating > 0 {
			...
			dp.logger.Info("🎬 [豆瓣评分] ✅ %s (IMDb=%s) → %.1f", name, imdbID, rating)
			return
		}
		if err != nil {
			dp.logger.Debug("🎬 [豆瓣评分] IMDb 查询失败 %s: %v", imdbID, err)
		}
	}
	...
	if name != "" {
		...
		if err == nil && rating > 0 {
			dp.storeCache(imdbID, name, year, rating)
			dp.statFetched.Add(1)
			dp.logger.Info("🎬 [豆瓣评分] ✅ %s (标题搜) → %.1f", name, rating)
			return
		}
	}

	dp.statFailed.Add(1)
	dp.logger.Debug("🎬 [豆瓣评分] ❌ %s 未匹配", name)
}
...
func (dp *DoubanProvider) FetchSeason(ctx context.Context, seriesName string, seasonNumber int) {
	...
	if err != nil || rating <= 0 {
		dp.statFailed.Add(1)
		dp.logger.Debug("🎬 [豆瓣评分] ❌ 季 %s 第%d季 未匹配", seriesName, seasonNumber)
		return
	}

	dp.storeCacheByKey(key, seriesName, rating)
	dp.statFetched.Add(1)
	dp.logger.Info("🎬 [豆瓣评分] ✅ %s 第%d季 → %.1f", seriesName, seasonNumber, rating)
}
...
//
// 策略：
//   - Movie / Series：直接覆盖 CommunityRating
...
func (dp *DoubanProvider) injectMovieOrSeries(item map[string]interface{}) {
	...
	dp.logger.Debug("🎬 [豆瓣评分] 注入 %s: CommunityRating=%.1f", name, rating)
}
...
func (dp *DoubanProvider) injectSeason(item map[string]interface{}) {
	...
	// 尝试：直接给 Season 加 CommunityRating（客户端可能不认，看实测）
	item["CommunityRating"] = rating

	// 兜底：Overview 前缀（所有客户端都会显示）
	...
	dp.logger.Debug("🎬 [豆瓣评分] 注入季 %s 第%d季: %.1f", seriesName, indexNumber, rating)
}
...
// globalRateLimit 全局限速（所有豆瓣请求串行）
func (dp *DoubanProvider) globalRateLimit(ctx context.Context) {
	dp.rateMu.Lock()
	elapsed := time.Since(dp.lastReq)
	var wait time.Duration
	if elapsed < doubanMinRequestInterval {
		wait = doubanMinRequestInterval - elapsed
	}
	dp.lastReq = time.Now().Add(wait)
	dp.rateMu.Unlock()
...
// getMovieRatingByImdb 电影 IMDb → 官方 API
...
// getTVRatingByImdb 电视剧 IMDb → 第三方 API
...
// searchByTitle 标题搜（无 IMDb 或 IMDb 失败时的兜底）
...
// searchSeasonRating 季评分搜索（剧名 + 季号）
...
// fetchRatingByDoubanID 豆瓣 ID → 官方 API 查详情
		...
		case <-ticker.C:
			if dp.dirty.Load() {
				if err := dp.saveDB(); err != nil {
					dp.logger.Warn("🎬 [豆瓣评分] 写盘失败: %v", err)
				}
			}
		case <-dp.stopCh:
			return
...
// normalizeTitle 归一化标题（去空格、小写），用于模糊匹配
